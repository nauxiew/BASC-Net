...
from net.SCA.SCA import SCA_attention


class BASC_v2(nn.Module):
    def __init__(self, cfg):
        super(BASC_v2, self).__init__()
        self.backbone = None		
        ...
        self.cls_conv = nn.Conv2d(cfg.MODEL_ASPP_OUTDIM, 3, 1, 1, padding=0)
        self.att_block = SCA_attention(in_channels=cfg.MODEL_ASPP_OUTDIM, out_channels=cfg.MODEL_ASPP_OUTDIM//2, K=8)
    ...
    def forward(self, x, edge=None, period=None):
        ...
        feature_aspp, consistent_loss = self.att_block(x=feature_aspp, edge =edge, period=period, scale_factor=1/4)
        ...
        if period is not None:
            ...
            return result, consistent_loss, feature
        else:
            return result
